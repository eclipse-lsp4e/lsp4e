...
public abstract class AbstractcSemanticTokensDataStreamProcessor<T, V> {
	 ...
	 */
	public List<V> getTokens(final List<Integer> dataStream,
		...
		for (Integer data : dataStream) {
			...
			case 4: // token modifier
				prevLine = line;
				@Nullable V token = createToken(tokenType == null ? null : tokenTypeMapper.apply(tokenType), offset, length, tokenModifiers(data, semanticTokensLegend.getTokenModifiers()));
				if (token != null) {
					tokens.add(token);
				}
				break;
	...
	abstract protected @Nullable V createToken(@Nullable T tokenType, int offset, int length, List<String> tokenModifiers);
